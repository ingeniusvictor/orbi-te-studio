...
import {
  renderLocationSketchPanel,
  type LocationSketchModel
} from "./location-sketch.js";
import { renderRic18TitleBlock } from "./render-ric18-title-block.js";

export function renderRic18Footer(
  titleBlock: TitleBlock,
  location: LocationSketchModel = { status: "pending" }
): string {
  const parts: string[] = [];
  ...
  parts.push(
    `<g transform="translate(0,8) scale(0.52)">${renderLocationSketchPanel(
      location
    )}</g>`
  );
  ...
  parts.push(
    `<g transform="translate(190,0)">${renderRic18TitleBlock(titleBlock)}</g>`
  );
